...
export default function JoybuyPage() {
  ...
  return <main className="page joybuy-page">
    ...
    <section className="catalogue joybuy-catalogue" aria-labelledby="joybuy-title">
      ...
      <div className="offer-grid joybuy-grid">{offers.map((offer) => <article className="offer joybuy-offer" key={offer.name}><div className="photo"><img src={offer.imageUrl} alt={offer.nameZh || offer.name} loading="lazy" decoding="async" /><span>−{offer.discountPercent}%</span></div><div className="offer-body"><p className="category">JOYBUY · 食品闪电优惠</p><h3>{chineseName(offer)}</h3><p className="pack">{offer.name}</p><p className="joybuy-meta">{offer.rating && `评分 ${offer.rating}`} {offer.rating && offer.sold && " · "}{offer.sold}</p><div className="price"><b>{euro.format(offer.sale)}</b>{offer.original && <s>{euro.format(offer.original)}</s>}</div><p className="advice">{judgement(offer)}</p><a className="product-link" href={offer.productUrl} target="_blank" rel="noreferrer">在 Joybuy 查看商品 ↗</a></div></article>)}</div>
    </section>
