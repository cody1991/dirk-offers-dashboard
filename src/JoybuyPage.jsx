...
export default function JoybuyPage() {
  ...
  return <main className="page joybuy-page">
    <header className="hero joybuy-hero">
      ...
      <div>
        <div className="hero-meta"><p className="kicker">荷兰 · 闪电优惠</p><a className="official-link" href={data.sourceUrl} target="_blank" rel="noreferrer">打开 Joybuy 闪电优惠 <span>↗</span></a></div>
        <h1>闪电价，<em>先看</em>再买。</h1>
        <p className="subhead">电子、家居与日用的 Joybuy 限时优惠，独立浏览，避免和超市清单混在一起。</p>
      </div>
    ...
    <section className="catalogue joybuy-catalogue" aria-labelledby="joybuy-title">
      <div className="catalogue-head"><div><span>当前闪电优惠 · {offers.length} 项</span><h2 id="joybuy-title">折扣先排好，再决定要不要点开。</h2></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索：平板、耳机、家居…" aria-label="搜索 Joybuy 优惠" /></div>
      <div className="offer-grid joybuy-grid">{offers.map((offer) => <article className="offer joybuy-offer" key={offer.productUrl}><div className="photo"><img src={offer.imageUrl} alt={offer.name} loading="lazy" decoding="async" /><span>−{offer.discountPercent}%</span></div><div className="offer-body"><p className="category">JOYBUY · FLASH DEAL</p><h3>{offer.name}</h3><p className="joybuy-meta">{offer.rating && `评分 ${offer.rating}`} {offer.rating && offer.sold && " · "}{offer.sold}</p><div className="price"><b>{euro.format(offer.sale)}</b>{offer.original && <s>{euro.format(offer.original)}</s>}</div><p className="advice">{offer.advice}</p><a className="product-link" href={offer.productUrl} target="_blank" rel="noreferrer">在 Joybuy 查看商品 ↗</a></div></article>)}</div>
    </section>
    <footer>数据来自 Joybuy 闪电优惠页；价格、库存及配送以 Joybuy 页面为准。</footer>
  </main>;
}
