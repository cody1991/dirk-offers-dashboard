...
export default function App() {
  ...
  return <main className="page">
    ...
    <section className="catalogue" aria-labelledby="catalogue-title">
      <div className="catalogue-head"><div><span>全部优惠 · {offers.length} 项</span><h2 id="catalogue-title">今日优惠。</h2><p className="catalogue-summary">按折扣、单位价格或售价排序。</p></div><div className="catalogue-controls"><label>排序<select value={sortBy} onChange={(e) => setSortBy(e.target.value)}><option value="discount">折扣最高</option><option value="unit">单位价格低</option><option value="price">售价最低</option></select></label><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="搜索：草莓、鸡翅、咖啡…" aria-label="搜索优惠" /></div></div>
      ...
      <div className="offer-grid">
        {offers.map((offer) => {
          ...
          return <article className={"offer " + (atHistoricalLow ? "historical-low" : "")} key={offer.name}>
            ...
            <div className="offer-body"><p className="category">{offer.category}</p><h3>{offer.name}</h3><p className="pack">{offer.package}</p><div className="price"><b><Price value={offer.sale} /></b>{offer.original && <s><Price value={offer.original} /></s>} {offer.discountPercent && <i>−{offer.discountPercent}%</i>}</div><UnitPrice offer={offer} /><PriceTrail history={offer.metrics} /><p className="advice">{offer.advice}</p>{offer.productUrl && <a className="product-link" href={offer.productUrl} target="_blank" rel="noreferrer">在 Dirk 查看原商品 ↗</a>}</div>
          </article>;
        })}
      </div>
    </section>
